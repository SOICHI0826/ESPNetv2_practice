...
Original file is located at
    https://colab.research.google.com/drive/14D7TZhIOt2wfYwtCAZinRTlK-hI8oyKL
"""
...
class EESP(nn.Module):
  def __init__(self, nIn, nOut, stride = 1, k = 4, rf_lim = 7, down_method = 'esp'):
    ...
    n = int(nOut / k)
    n1 = nOut - (k - 1) * n
    self.proj_11 = CBR(nIn, n, 1, stride = 1, groups = k)
...
class EESPNet(nn.Module):
  def __init__(self, classes = 20, s = 1):
    ...
    for i in range(reps[1]):
      self.level3.append(EESP(config[2], config[2], stride = 1, k = K[2], rf_lim = rf_lim[2]))

    self.level4_0 = DownSampler(config[2], config[3], k = K[2], rf_lim = rf_lim[2], reinf = self.input_reinforcement)
    self.level4 = nn.ModuleList()
    for i in range(reps[2]):
      self.level4.append(EESP(config[3], config[3], stride = 1, k = K[2], rf_lim = rf_lim[3]))

    self.level5_0 = DownSampler(config[3], config[4], k = K[3], rf_lim = rf_lim[3])
    self.level5 = nn.ModuleList()
    for i in range(reps[3]):
      self.level5.append(EESP(config[4], config[4], stride = 1, k = K[4], rf_lim = rf_lim[4]))
